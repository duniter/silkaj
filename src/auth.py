...
def auth_by_wif():
    ...
    if fi == b'\x01':
        return get_seed_from_wif(wif)
    elif fi == b'\x02':
        password = getpass.getpass("Please enter the password of WIF (masked): ")
        return get_seed_from_ewif(wif,password)
...
def get_seed_from_ewif(ewif,password):
    regex = re.compile('^[1-9A-HJ-NP-Za-km-z]*$')
    ...
    #Checksum Control
    checksum = nacl.hash.sha256(nacl.hash.sha256(wif_no_checksum, nacl.encoding.RawEncoder),nacl.encoding.RawEncoder)[0:2]
    ...
    #Password Control
    salt_from_seed =  nacl.hash.sha256(nacl.hash.sha256(b58_decode(get_publickey_from_seed(seedhex)), nacl.encoding.RawEncoder), nacl.encoding.RawEncoder)[0:4]
